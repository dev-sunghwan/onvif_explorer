...
## Enhancement #4 - ONVIF 프로파일 지원 여부 확인 기능 (2026-02-23)

### 변경 내용
연결된 카메라가 지원하는 ONVIF 프로파일(S/T/G/C/A/D/M/Q)을 자동으로 감지하는 "Check Profiles" 기능 추가.

### 추가/수정 파일

**`onvif_client/profile_checker.py`** (신규)
- `ProfileChecker` 클래스 구현
- `GetServices(IncludeCapability=False)` 호출로 지원 서비스 네임스페이스 수집
- 구형 펌웨어 대응: `GetServices` 실패 시 `GetCapabilities` 폴백
- `PROFILE_DEFINITIONS`: 프로파일 키(S/T/G/C/A/D/M/Q)별 필수 네임스페이스 매핑
- 반환값: `{success, services, profiles, profile_details, error}`

**`app.py`**
- `POST /api/check-profiles` 라우트 추가
- `VERSION = "0.1.3"` 상수 추가 (버전 관리 단일 진실 공급원)
- `index()`: `version=VERSION` 템플릿 전달

**`templates/index.html`**
- 네비게이션 바에 버전 배지 추가 (`v{{ version }}`)
- "Check Profiles" 버튼 추가 (Test Connection 옆)
- 프로파일 확인 결과 모달 추가

**`static/js/app.js`**
- `checkProfiles()`, `renderProfileResult()` 함수 추가
- "Check Profiles" 버튼 이벤트 리스너 추가

**`onvif_tester.spec`**
- `app.py`에서 `VERSION`을 동적으로 읽어 exe 파일명에 반영
- 출력: `ONVIF_Command_Tester_v{version}.exe`

---

## Enhancement #5 - Operation 필터 기능 (2026-02-23)

### 변경 내용
Operation 드롭다운 위에 검색/필터 입력창 추가. 오퍼레이션이 많은 서비스(Device: 103개, Media: 79개 등)에서 원하는 오퍼레이션을 빠르게 찾을 수 있음.

### 수정 파일

**`templates/index.html`**
- Operation 드롭다운 위에 필터 입력창 + 클리어(X) 버튼 추가

**`static/js/app.js`**
- `applyOperationFilter()`: 입력 텍스트 기반 실시간 필터링, 이전 선택값 유지
- 바인딩 변경 시 필터 초기화

---

## Enhancement #6 - Last Response Values 패널 + required 필드 자동 펼침 (2026-03-14)

### 변경 내용
1. 마지막 성공 응답값을 왼쪽 패널 하단에 key-value 표로 표시, 값별 복사 버튼 제공
2. `required`인 복합 파라미터 필드셋을 폼 로드 시 자동으로 펼쳐서 필수 입력값을 놓치지 않도록 개선
3. `@` prefix 속성 파라미터를 zeep에 전달 시 자동으로 prefix 제거

### 배경
`SetMetadataConfiguration` 테스트 중 `Multicast.Address.Type`(required enum)이 기본 접힌 상태로 인해 선택되지 않아 "Missing element Address" 에러가 반복 발생. Get* 응답값을 수동으로 찾아 Set* 파라미터에 입력하는 불편함도 있었음.

### 수정 파일

**`static/js/param-builder.js`**
- `_buildComplexField()`: `param.required === true`이면 `childContainer` 기본 펼침, 토글 버튼 아이콘도 초기값 반영
- `collectParams()`: 각 경로 세그먼트에서 `@` prefix 제거 (zeep은 `token` 형식 기대, UI는 `@token` 표시)

**`templates/index.html`**
- "Last Response Values" 카드 추가 (왼쪽 패널 하단, 기본 숨김)

**`static/js/app.js`**
- `flattenJson()`: JSON 재귀 평탄화 (null 값 제외, 배열 인덱스 포함)
- `displayResponseValues()`: 평탄화된 key-value 테이블 렌더링, 복사 버튼 이벤트 등록
- `displayResult()` 내 `displayResponseValues()` 호출 추가 (성공 시 표시, 실패 시 숨김)
- X 버튼으로 패널 수동 닫기

**`static/css/style.css`**
- `.response-values-scroll`: 최대 높이 280px + 세로 스크롤
